...
BATCH_SIZE = 64  # העלינו את ה-Batch Size כי המודל קל יותר, יאיץ את הריצה
EPOCHS = 2
...
def main():
    ...
    val_transform = transforms.Compose([
        transforms.Resize(int(model.IMAGE_SIZE * 1.14)),  # קצת יותר גדול לפני חיתוך
        transforms.CenterCrop(model.IMAGE_SIZE),
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ])

    full_train_dataset = ImageNetSubset(DATA_ROOT, r"train_set\train", transform=train_transform)
    full_val_dataset = ImageNetSubset(DATA_ROOT, r"train_set\train", transform=val_transform)
    ...
    for epoch in range(EPOCHS):
        last_loss = train_one_epoch(epoch, writer, model, optimizer, train_loader, device)
